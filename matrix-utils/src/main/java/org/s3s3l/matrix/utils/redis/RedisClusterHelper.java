...
import org.s3s3l.matrix.utils.redis.configuration.HAPClusterNode;
...
public class RedisClusterHelper implements InitializableRedis<HAPClusterNode> {
    ...
    private StructuralHelper jsonHelper = JacksonUtils.DEFAULT;
    private JedisCluster cluster;

    public static RedisClusterHelper create(HAPClusterNode clusterConfig) {
        RedisClusterHelper operation = new RedisClusterHelper();
        operation.cluster = new JedisCluster(clusterConfig.getClusterConfig().stream()
                .map(r -> new HostAndPort(r.getHost(), r.getPort()))
                .collect(Collectors.toSet()), clusterConfig.getConnectionTimeout(),
                clusterConfig.getSoTimeout(),
                clusterConfig.getMaxAttempts(), clusterConfig.getPassword(), clusterConfig.getPoolConfig());
        return operation;
    }
    ...
    public void init(HAPClusterNode configuration) {
        cluster = new JedisCluster(configuration.getClusterConfig()
                .stream()
                .map(r -> new HostAndPort(r.getHost(), r.getPort()))
                .collect(Collectors.toSet()), configuration.getConnectionTimeout(),
                configuration.getSoTimeout(),
                configuration.getMaxAttempts(), configuration.getPassword(), configuration.getPoolConfig());
    }
    ...
    public List<String> brpop(int timeout, String... keys) {
        return execute(jedis -> jedis.brpop(timeout, keys));
    }
